(function(){
  function ready(){
    ...
    saveBiz=async function(id,box){
      ...
      try{validateMedia(kept,files);st.textContent=files.length?'جاري رفع الصور والفيديو وحفظ التعديلات...':'جاري حفظ التعديلات...';st.className='status ok';data.newMedia=[];for(var i=0;i<files.length;i++)data.newMedia.push(await fileToPayload(files[i]));await api('businessAdsSession',data);st.textContent='تم حفظ التعديلات والوسائط.';st.className='status ok';await loadBusinessAds();}catch(e){st.textContent=e.message;st.className='status err';}
    };
  }
  ready();
})();